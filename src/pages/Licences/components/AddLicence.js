...
import {
  ...
  getUsersNameAction,
} from "../../../stores/actions/picker/titlePicker.action";
...
export default function AddLicence() {
  ...
  return (
    <MainContainer>
      <SubHeading>Add new license</SubHeading>
      <Content>Add license</Content>
      ...
      <form onSubmit={handleSubmit(handleAddLicense)}>
        <Grid container direction="column" className="mt-4">
          ...
          <FormControl component="fieldset">
            ...
            <RadioGroup
              ...
              onChange={(e) =>
                setState({ ...state, licenseType: e.target.value })
              }
            >
              <RadioLabel
                value="Company"
          ...
          <Grid item xs={12} md={6} style={{ marginTop: "15px" }}>
            <AppAutoComplete
              ...
              loading={
                state.licenseType === "Company"
                  ? company?.companySearch?.loading
                  : user?.userSearch?.loading
              }
              data={
                state.licenseType === "Company"
                  ? company?.companySearch?.data?.docs
                  : user?.userSearch?.data?.docs
              }
              error={
                state.licenseType === "Company"
                  ? company?.companySearch?.error
                  : user?.userSearch?.error
              }
          ...
          <Grid item xs={12} md={4}>
            <StyledTextField
              ...
              type="number"
              value={
                state.isUnlimitedEncode
                  ? Number.POSITIVE_INFINITY
                  : state?.maxEncodeUses
              }
              placeholder={state.isUnlimitedEncode ? "Unlimited" : "eg. 100"}
              onChange={(e) =>
                setState({ ...state, maxEncodeUses: e.target.value })
              }
              style={{ marginTop: "15px" }}
              InputLabelProps={{
                shrink: true,
          ...
          <Grid item xs={12} md={4}>
            <StyledTextField
              ...
              type="number"
              value={
                state.isUnlimitedMonitor
                  ? Number.POSITIVE_INFINITY
                  : state?.maxMonitoringUses
              }
              placeholder={state.isUnlimitedMonitor ? "Unlimited" : "eg. 100"}
              onChange={(e) =>
                setState({ ...state, maxMonitoringUses: e.target.value })
              }
              style={{ marginTop: "15px" }}
              InputLabelProps={{
                shrink: true,
        ...
        <Content className="mt-4">Status</Content>
        ...
        <Grid container spacing={2} className="mt-1">
          <Grid item>
            <AppToggleSwitch
              ...
              onChange={(e) =>
                setState({ ...state, isUnlimitedEncode: e.target.checked })
              }
            />
          </Grid>
          ...
          <Grid item>
            <AppToggleSwitch
              ...
              onChange={(e) =>
                setState({ ...state, isUnlimitedMonitor: e.target.checked })
              }
            />
          </Grid>
          ...
          <Grid item>
            <AppToggleSwitch
              ...
              onChange={(e) =>
                setState({ ...state, isSuspended: e.target.checked })
              }
            />
          </Grid>
        </Grid>
        ...
        <Grid container className="mt-3 mb-2" justifyContent="flex-end">
          <AppButton
            variant={"outline"}
            onClick={() => navigate(-1)}
            disabled={license?.addLicenseKey?.loading}
          >
            Cancel
          </AppButton>
          <AppButton
