...
export default function Profile() {
  ...
  const schema = Yup.object().shape({
    ...
    newPassword: Yup.string()
      ...
      .min(
        6,
        "Min. 6 characters, max. 98 characters, atleast one special character,uppercase and lowercase"
      )
      .max(
        98,
        "Min. 6 characters, max. 98 characters, atleast one special character,uppercase and lowercase"
  ...
  const getType = () => {
    if (
      profile?.userProfile?.data?.userRole === userRoles.PARTNER_ADMIN ||
      profile?.userProfile?.data?.userRole === userRoles.PARTNER_USER
    ) {
      return "Partner";
    } else if (
      profile?.userProfile?.data?.userRole === userRoles.COMPANY_ADMIN ||
      profile?.userProfile?.data?.userRole === userRoles.COMPANY_USER ||
      profile?.userProfile?.data?.userRole === userRoles.PORTAL_USER
    ) {
      return "Company";
    }
    return null;
  ...
  return (
    <MainContainer>
      ...
      <Content>Update {getAccountType()} details</Content>
      ...
      <form onSubmit={handleSubmit(updateProfile)}>
        <Grid container spacing={6} className="mt-2">
          <Grid item xs={12} md={6}>
            ...
            <Grid style={{ marginTop: 15 }}>
              <DisabledTextField label={"User Type"} value={getUserType()} />
            </Grid>
          ...
          <Grid item xs={12} md={6}>
            {(getType() !== null || getUserType !== "Portal User") && (
              <Grid className="mb-4">
                ...
                <>
                  {getUserType() === "Partner Admin" ||
                  getUserType() === "Partner User" ? (
                    <Grid>
                      <Grid style={{ marginTop: 15 }}>
                        <DisabledTextField
                          label={"Partner Name"}
                          value={
                            profile?.userProfile?.data?.partner?.name || ""
                          }
                        />
                      </Grid>
                      <Grid style={{ marginTop: 15 }}>
                        <DisabledTextField
                          label={"Partner Type"}
                          value={
                            profile?.userProfile?.data?.partner?.partnerType ||
                            ""
                          }
                        />
                      </Grid>
                    ...
                    <Grid>
                      <Grid style={{ marginTop: 15 }}>
                        <DisabledTextField
                          label={"Company Name"}
                          value={
                            profile?.userProfile?.data?.company?.name || ""
                          }
                        />
                      </Grid>
                      ...
                      <Grid style={{ marginTop: 15 }}>
                        <DisabledTextField
                          label={"Company Type"}
                          value={
                            profile?.userProfile?.data?.company?.companyType ||
                            ""
                          }
                        />
                      </Grid>
                      ...
                      <Grid style={{ marginTop: 15 }}>
                        <DisabledTextField
                          ...
                          value={
                            profile?.userProfile?.data?.company
                              ?.companyUrnOrId || ""
                          }
                        />
                      </Grid>
