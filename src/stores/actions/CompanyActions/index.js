...
import { createCompany, getAllCompanies, getCompanyEncodes, getCompanyEncodesExports, updateCompanyProfile } from "../../../services/https/resources/Companies/Companies.api";
...
export const getCompanySearchAction = (companyName, limit = "50") => {
    ...
    return (dispatch) => {
        ...
        getAllCompanies(params).then((response) => {
            ...
            dispatch({ type: actionTypes.SET_SEARCH_COMPANY_ERROR, data: err?.message })
            cogoToast.error(err?.message)
        })
    }
}

export const getCompanyEncodesAction = (limit = 10, page = 1, sortBy, isAscending) => {
    let companyEncodes = store.getState()?.company?.companyEncodes;
    let userRoleWiseId = getRoleWiseID()

    let newEndDate = moment(companyEncodes?.dates?.endDate).endOf("days").toISOString()
    let params = new URLSearchParams(`createdAt>=${moment(companyEncodes?.dates?.startDate).format("YYYY-MM-DD")}&createdAt<=date(${newEndDate})`)
    ...
    return (dispatch) => {
        ...
        getCompanyEncodes(params).then((res) => {
            dispatch({ type: actionTypes.GET_COMPANY_ENCODES_DATA, data: res })
        }).catch((err) => {
            dispatch({ type: actionTypes.GET_COMPANY_ENCODES_ERROR, data: err?.message })
        })
    }
}

export const getCompanyEncodesExportsAction = (format, limit = 10, page = 1, sortBy, isAscending) => {
    let companyEncodes = store.getState()?.company?.companyEncodes;
    let userRoleWiseId = getRoleWiseID()
    ...
    return (dispatch) => {
        getCompanyEncodesExports(format, params).then((data) => {
            log("Company Encodes graphs", data);
            if (format === "xlsx") {
                fileDownload(data, `Encodes_By_Company_Export-xlsx-(${moment(companyEncodes?.dates?.startDate).format("YYYY_MM_DD")}-to-${moment(companyEncodes?.dates?.endDate).format("YYYY_MM_DD")})_${format}.xlsx`);
            } else {
                fileDownload(data, `Encodes_By_company_Export-csv-(${moment(companyEncodes?.dates?.startDate).format("YYYY_MM_DD")}-to-${moment(companyEncodes?.dates?.endDate).format("YYYY_MM_DD")})_${format}.csv`);
            }
        }).catch(error => {
