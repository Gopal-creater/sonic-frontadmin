  ...
  produce(state, (draft) => {
    switch (action.type) {
      ...
      case actionTypes.GET_USERPROFILE_DATA:
        ...
        if (action.data.userRole === userRoles.COMPANY_ADMIN) {
          draft.userMenus = draft.userMenus.filter((menu) => {
            if (menu.urlName === "Companies" || menu.urlName === "User Profile")
              return null;
            return menu;
          });
        }
        ...
        if (action.data.userRole === userRoles.PARTNER_ADMIN) {
          draft.userMenus = draft.userMenus.filter((menu) => {
            if (menu.urlName === "User Profile") return null;
            return menu;
          });

          draft.sideBarData = draft.sideBarData.filter((item) => {
            if (item.path === "/encode" || item.path === "/decode") return null;
            return item;
          });
        }

        if (
          action.data.userRole === userRoles.COMPANY_USER ||
          action.data.userRole === userRoles.PARTNER_USER ||
          action.data.userRole === userRoles.PORTAL_USER
        ) {
          draft.userMenus = draft.userMenus.filter((menu) => {
            if (
              menu.urlName === "Admin Profile" ||
              menu.urlName === "Users" ||
              menu.urlName === "Companies"
            )
              return null;
            return menu;
          });
      ...
      case actionTypes.UPDATE_USERS_PROFILE:
        let index = draft.getUsers.data?.docs?.findIndex(
