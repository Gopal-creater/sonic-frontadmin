...
const SonicKeys = (props) => {
  ...
  const firstFetchSonicKey = (_offset = 0, _limit = 10, value = "") => {
    Communication.fetchMySonicKey(_limit, _offset, value)
      .then((res) => {
  ...
  const handleClickOpenTable = async (data) => {
    ...
    setSonicKeys({
      ...
      createdAt: isValid(new Date(data.createdAt))
      ...
      contentSamplingFrequency: data?.contentSamplingFrequency?.replace(
        "Hz",
        ""
      ),
      ...
      job: data?.job,
      additionalMetadata: data?.additionalMetadata?.message
                  ...
                  return (
                    <TableRow className={classes.tableRow} key={index}>
                      ...
                      <TableCell className={classes.sonicKeyText}>
                        {isSelected("SONICKEY") && data.sonicKey}
                      </TableCell>
                      <TableCell className={classes.tableCellNormalText}>
                        {isSelected("NAME") && data.contentFileName}
                      </TableCell>
                      <TableCell className={classes.tableCellNormalText}>
                        {isSelected("ARTIST") &&
                          (data.contentOwner === "" ? "-" : data.contentOwner)}
                      </TableCell>
                      <TableCell className={classes.tableCellNormalText}>
                        {isSelected("ENCODED DATE") &&
                          format(
                            new Date(data.contentCreatedDate),
                            "dd/MM/yyyy"
                          )}
                      </TableCell>
                      <TableCell className={classes.tableCellNormalText}>
